...
export const Login = () => {
    ...
    const handleEmailChange = useCallback(
        (email: string) => setFormData((prev) => ({ ...prev, email })),
        []
    );
        ...
        async (password: string) => {
            try {
                ...
                if (
                    ...
                    setTimeout(() => router.push("/"), 2000); // для показа сплеш скрина
                } else {
                    router.push("/");
                }
            } catch (err: unknown) {
    ...
    return (
        <AuthLayout>
            {step === 0 && (
                <LoginEmail onChange={handleEmailChange} onNext={goNext} />
            )}
            {step === 1 && (
                <LoginPassword onNext={handleFinish} onPrev={goBack} />
            )}
