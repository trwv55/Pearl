...
export default function DaysSwitcher({ value, onChange }: DaysSwitcherProps) {
	...
	const { monthTitle, yearTitle } = useMemo(() => {
		const raw = format(selectedDate, "LLL", { locale: ru });
	...
	useEffect(() => {
		...
		const handleScroll = () => {
			...
			if (scrollLeft < 60) prependDays(BATCH_SIZE);
			if (scrollLeft + clientWidth > scrollWidth - 60) appendDays(BATCH_SIZE);
		};
